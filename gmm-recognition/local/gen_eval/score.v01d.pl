...
sub read_trials {
	...
	if (! /^#/ && ! /^\s*$/) {
	    ...
	    if ($fields[1] !~ /^(french|italian|polish|arabic|bengali|chinese|cantonese|mandarin|mainland|taiwan|min|wu|english|american|indian|hindustani|hindi|urdu|spanish|caribbean|noncaribbean|farsi|german|japanese|korean|russian|tamil|thai|vietnamese)$/i) {
...
sub is_out_of_set_language {
    ...
    if ( $test eq "general_lr" ) {
	if ($lng =~ /(arabic|bengali|farsi|german|japanese|korean|russian|tamil|french|polish|italian|thai|vietnamese|chinese|english|hindustani|spanish)/i) {
	    return 0;
	}
