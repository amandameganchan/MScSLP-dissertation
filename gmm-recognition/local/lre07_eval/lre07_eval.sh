...
for dur in "3" "10" "30"; do
  utils/filter_scp.pl -f 1 data/lre07_new/"$dur"sec \
  ...
  local/lre07_eval/lre07_targets.pl $posterior_dir/posteriors_"$dur"sec \
    <(utils/filter_scp.pl -f 1 data/lre07_new/"$dur"sec data/lre07_new/utt2lang) \
    $languages_file \
...
er=$(compute-wer --text ark:<(lid/remove_dialect.pl data/lre07_new/utt2lang) \
  ark:$posterior_dir/output 2>/dev/null | grep "WER" | awk '{print $2 }')
printf '% 7.2f' $er

for dur in "3" "10" "30"; do
  er=$(compute-wer --text ark:<(utils/filter_scp.pl -f 1 \
    data/lre07_new/"$dur"sec data/lre07_new/utt2lang | lid/remove_dialect.pl -) \
    ark:<(utils/filter_scp.pl -f 1 data/lre07_new/"$dur"sec \
