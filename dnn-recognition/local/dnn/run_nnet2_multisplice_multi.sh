...
num_threads=1
minibatch_size=512 
dir=exp/dnn_multi/nnet2_online/nnet_ms_a
mkdir -p exp/dnn_multi/nnet2_online


# Stages 1 through 5 are done in run_nnet2_common.sh,
# so it can be shared with other similar scripts.
local/dnn/run_nnet2_common_multi.sh --stage $stage
...
if [ $stage -le 6 ]; then
  ...
  lid/nnet2/train_multisplice_accel2.sh --stage $train_stage \
    ...
    --pnorm-input-dim 1024 \
    --pnorm-output-dim 128 \
    data/NEW_dnn_multi/train_hires_asr data/NEW_dnn_multi/lang exp/dnn_multi/tri4a $dir  || exit 1;
