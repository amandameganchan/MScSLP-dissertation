...
while($line = <POSTS>) {
  ...
  if ($lang =~ /(french|italian|polish|arabic|bengali|farsi|german|japanese|korean|russian|tamil|thai|vietnamese|chinese|english|hindustani|spanish)/i) {
  ...
  for ($i = 1; $i < $size; $i++) {
    ...
    if ($nontarget_lang =~ /(arabic|bengali|french|italian|polish|farsi|german|japanese|korean|russian|tamil|thai|vietnamese|chinese|english|hindustani|spanish)/i) {
      $prob = exp($toks[$i]);
